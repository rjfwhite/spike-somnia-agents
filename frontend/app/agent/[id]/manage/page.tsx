...
function ManageContent({ agentId }: { agentId: string }) {
    ...
    const AGENT_REGISTRY_ADDRESS = currentNetwork.contracts.agentRegistry;

    // Read initial values from URL query params (from CLI publish)
    const initialMetadataUri = searchParams.get('metadataUri') || '';
    const initialContainerImageUri = searchParams.get('containerImageUri') || '';
    const hasInitialValues = initialMetadataUri || initialContainerImageUri;
    ...
    const [metadataMode, setMetadataMode] = useState<InputMode>(initialMetadataUri ? 'url' : 'upload');
    const [metadataUrl, setMetadataUrl] = useState(initialMetadataUri);

    // Container image mode - default to 'url' if we have a containerImageUri, otherwise 'upload'
    const [containerMode, setContainerMode] = useState<InputMode>(initialContainerImageUri ? 'url' : 'upload');
    const [containerUrl, setContainerUrl] = useState(initialContainerImageUri);

    // Pre-fill from existing agent data
    useEffect(() => {
        if (agentExists && !initialMetadataUri && !initialContainerImageUri) {
            setMetadataUrl(agent.metadataUri);
            setContainerUrl(agent.containerImageUri);
            setMetadataMode('url');
            setContainerMode('url');
        }
    }, [agentExists, agent, initialMetadataUri, initialContainerImageUri]);
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-400 mt-1">Create or update this agent on the registry</p>
                    </div>
                </div>
    ...
    return (
        <div className="space-y-6">
            ...
            {canEdit && (
                <div className="bg-slate-900/50 border border-white/10 rounded-lg p-6">
                    ...
                    <p className="text-gray-400 text-sm mb-6">
                        {agentExists
                            ? "Update the agent's metadata and container image URI."
                    ...
                    {isSetConfirmed && setAgentHash && (
                        <div className="mt-4 p-4 rounded-lg border bg-green-500/10 border-green-500/20">
                            ...
                            <a
                                href={`${currentNetwork.explorerUrl}/tx/${setAgentHash}`}
                                target="_blank"
                                rel="noopener noreferrer"
            ...
            <div className="bg-slate-900/50 border border-white/10 rounded-lg p-4">
                ...
                <p className="font-mono text-sm text-green-400 break-all">{AGENT_REGISTRY_ADDRESS}</p>
                <p className="text-xs text-gray-500 mt-1">{currentNetwork.name}</p>
            </div>
        </div>
    );
}
